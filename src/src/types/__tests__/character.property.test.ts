...
const weaponArb: fc.Arbitrary<Weapon> = fc.record({
  ...
  properties: fc.array(fc.string({ minLength: 1, maxLength: 20 }), {
    maxLength: 4,
  }),
...
const rechargeArb = fc.constantFrom<"short_rest" | "long_rest">(
  "short_rest",
  "long_rest"
);
...
const classResourcesArb: fc.Arbitrary<ClassResources> = fc.record(
  {
    ...
    preparedSpells: fc.array(fc.string({ minLength: 1, maxLength: 20 }), {
      maxLength: 10,
    }),
    autoPreparedSpells: fc.array(fc.string({ minLength: 1, maxLength: 20 }), {
      maxLength: 5,
    }),
...
const statsArb: fc.Arbitrary<
  Record<AbilityName, { value: number; modifier: number }>
> = fc.record({
  STR: abilityScoreArb,
  DEX: abilityScoreArb,
  CON: abilityScoreArb,
...
const coinsArb = fc.record({
  cp: fc.integer({ min: 0, max: 9999 }),
  sp: fc.integer({ min: 0, max: 9999 }),
  ep: fc.integer({ min: 0, max: 9999 }),
  gp: fc.integer({ min: 0, max: 9999 }),
...
const characterDataArb: fc.Arbitrary<CharacterData> = fc.record({
  ...
  // Feats
  featsTraits: fc.array(fc.string({ minLength: 1, maxLength: 30 }), {
    maxLength: 10,
  }),
  ...
  cantrips: fc.array(fc.string({ minLength: 1, maxLength: 30 }), {
    maxLength: 8,
  }),
...
describe("CharacterData JSON round-trip", () => {
  it("Property 1: Round-trip JSON serialization preserves all CharacterData fields", () => {
     ...
     * **Validates: Requirements 15.2, 15.7**
     *
     * Serializing a CharacterData object to JSON and parsing it back
     * must produce a deeply equal object. This ensures the data model
     * is fully JSON-safe (no functions, Dates, undefined-in-arrays, etc.)
     */
    fc.assert(
