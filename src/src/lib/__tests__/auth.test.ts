...
describe("Auth — middleware matcher", () => {
  // Import the matcher config from middleware to verify public routes are excluded
  // We test the regex pattern directly since middleware.ts exports a config object
  it("matcher excludes /login from auth guard", () => {
    // The matcher pattern: /((?!login|api/auth|_next/static|_next/image|favicon.ico|images).*)
