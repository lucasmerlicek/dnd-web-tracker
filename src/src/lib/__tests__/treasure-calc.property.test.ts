...
/** Generate a single valid TreasureItem with a non-negative estimatedValue. */
...
/** Generate an array of TreasureItems (possibly empty). */
...
describe("Feature: dnd-tracker-enhancements, Property 13: Treasure total value calculation", () => {
   ...
   *
   * For any list of treasure items with non-negative estimatedValue fields,
   * totalTreasureValue should return the exact sum of all estimatedValue fields.
   * Adding or removing an item should change the total by exactly that item's value.
   */
