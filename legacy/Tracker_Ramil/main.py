...
class ModernButton:
    def __init__(
        ...
        if aux_toggle is None and has_advantage_checkbox:
            aux_toggle = {"label": "ADV", "state": False}

        self.aux_toggle_enabled = aux_toggle is not None
        self.aux_toggle_label = (aux_toggle or {}).get("label", "ADV")
        self.aux_toggle_state = (aux_toggle or {}).get("state", False)
        self.aux_toggle_locked = (aux_toggle or {}).get("locked", False)
    ...
    def draw(self, screen):
        ...
        toggle_width = button_rect.width // 4 if self.aux_toggle_enabled else 0
        text_area_rect = pygame.Rect(button_rect.x, button_rect.y, button_rect.width - toggle_width, button_rect.height)
        ...
        if self.aux_toggle_enabled:
            toggle_x = button_rect.right - toggle_width
            toggle_rect = pygame.Rect(toggle_x, button_rect.y, toggle_width, button_rect.height)
            
            # Calculate different shade for advantage section
            if self.aux_toggle_state:
                # Orange highlight when checked
                if self.ui_box_button_image:
                    # Draw UI box image for advantage section with darker tint
                    adv_ui_box = pygame.transform.scale(self.ui_box_button_image, (toggle_width, button_rect.height))
                    adv_ui_box.set_alpha(alpha if 'alpha' in locals() else 180)
                    screen.blit(adv_ui_box, (toggle_x, button_rect.y))
                    # Orange tint overlay
                    adv_tint = pygame.Surface((toggle_width, button_rect.height), pygame.SRCALPHA)
                    pygame.draw.rect(adv_tint, (255, 165, 80, 90), (0, 0, toggle_width, button_rect.height), border_radius=8)
                    screen.blit(adv_tint, (toggle_x, button_rect.y))
                else:
                    adv_color = (min(255, int(self.color[0] + 60)), min(255, int(self.color[1] + 40)), min(255, int(self.color[2] + 20)))
                    pygame.draw.rect(screen, adv_color, toggle_rect, border_radius=8)
            else:
                ...
                if self.ui_box_button_image:
                    ...
                    adv_tint = pygame.Surface((toggle_width, button_rect.height), pygame.SRCALPHA)
                    pygame.draw.rect(adv_tint, (255, 255, 255, 20), (0, 0, toggle_width, button_rect.height), border_radius=8)
                    screen.blit(adv_tint, (toggle_x, button_rect.y))
                else:
            ...
            label_text = self.aux_toggle_label or "ADV"
            adv_text = adv_font.render(label_text, True, (255, 255, 255))
            adv_text_rect = adv_text.get_rect(center=(toggle_x + toggle_width // 2, button_rect.centery))
    ...
    def handle_click(self, pos):
        """Handle button click, returns ('button', True/False) or ('toggle', state)"""
        if not self.rect.collidepoint(pos):
            return ('none', False)

        if self.aux_toggle_enabled:
            toggle_width = self.rect.width // 4
            toggle_x = self.rect.right - toggle_width

            if pos[0] >= toggle_x and not self.aux_toggle_locked:
                self.aux_toggle_state = not self.aux_toggle_state
                return ('toggle', self.aux_toggle_state)
            elif pos[0] >= toggle_x and self.aux_toggle_locked:
                return ('toggle_locked', self.aux_toggle_state)
...
class DnDCharacterTool:
    ...
    def __init__(self):
        ...
        self.current_base_ac = 0
        self.bladesong_indicator_rect = None
        self.mage_armor_indicator_rect = None
        
        # Action system
        self.selected_action = None
        self.last_action_roll = None
        self.bladesong_active = False
        self.bladesong_uses_remaining = 0
        self.bladesong_max_uses = 0
        ...
        self.backspace_repeat_delay = 500  # Initial delay in ms
        self.backspace_repeat_rate = 50   # Repeat rate in ms
        
        # Shield spell AC tracking
        self.shield_active = False
        self.base_ac = None  # Will be set from character data

        # Shadow Blade concentration tracking
        self.shadow_blade_active = False
        self.shadow_blade_weapon = {
            "name": "Shadow Blade",
            "damage_dice": "2d8",
            "damage_type": "psychic",
            "attack_stat": "DEX",
        ...
        self.stat_rects = []  # Store ability score rectangles for click detection
    ...
    def create_default_character(self):
        """Create default character data (Ramil al-Sayif)"""
        return {
            "character_name": "Ramil al-Sayif",
            "race": "Human",
            "char_class": "Fighter 1",
            "level": 1,
            "current_hp": 12,
            "max_hp": 12,
            "ac": 15,
            ...
            "shield_active": False,
            "proficiency_bonus": 2,
            "bladesong_active": False,
            "bladesong_uses_remaining": 0,
            "bladesong_max_uses": 0,
            ...
            "weapons": [
                {
                    "name": "Scimitar (Nick Mastery)",
                    "damage_dice": "1d6",
                    "damage_type": "slashing",
                    "attack_stat": "DEX",
                    "properties": ["finesse", "light", "nick_mastery"],
                    "magic_bonus": 0,
                    "uses_dueling": False,
                    "two_handed": False
                },
                {
                    "name": "Shortsword (Vex Mastery)",
                    "damage_dice": "1d6",
                    "damage_type": "piercing", 
                    "attack_stat": "DEX",
                    "properties": ["finesse", "light", "vex_mastery"],
                    "magic_bonus": 0,
                    "uses_dueling": False,
                    "two_handed": False
                },
                {
                    "name": "Rapier (Vex Mastery)",
                    "damage_dice": "1d8",
                    "damage_type": "piercing",
                    "attack_stat": "DEX", 
                    "properties": ["finesse", "vex_mastery"],
                    "magic_bonus": 0,
                    "uses_dueling": False,
                    "two_handed": False
                }
            ...
            "save_proficiencies": ["STR", "CON"],  # Fighter gets STR and CON save proficiency
            "death_saves": {
                "successes": 0,
                "failures": 0
            },
            "actions": {
                "second_wind": {
                    "name": "Second Wind",
                    "description": "Heal 1d10 + 1 HP as Bonus Action",
                    "dice": "1d10",
                    "bonus": 1,  # Fighter level
                    "available": True,
                    "recharge": "short_rest"  # Recharges on short or long rest
                }
            },
            "inventory": {
                "gear": [
            ...
            "journal": {
                "sessions": {
                    "Session 1": "First adventure begins...\n\nRamil al-Sayif's journey starts here.\n\nA learned swordsman and scholar who blends precision and intellect in battle."
                },
            ...
            "place_page": 0,
            "skills": [
                {"name": "Acrobatics", "stat": "DEX", "proficient": False, "modifier": 3},
                {"name": "Animal Handling", "stat": "WIS", "proficient": False, "modifier": 1},
                {"name": "Arcana", "stat": "INT", "proficient": True, "modifier": 5},
                {"name": "Athletics", "stat": "STR", "proficient": False, "modifier": -1},
                {"name": "Deception", "stat": "CHA", "proficient": False, "modifier": -1},
                {"name": "History", "stat": "INT", "proficient": True, "modifier": 5},
                {"name": "Insight", "stat": "WIS", "proficient": True, "modifier": 3},
                {"name": "Intimidation", "stat": "CHA", "proficient": False, "modifier": -1},
                {"name": "Investigation", "stat": "INT", "proficient": False, "modifier": 3},
                {"name": "Medicine", "stat": "WIS", "proficient": False, "modifier": 1},
                {"name": "Nature", "stat": "INT", "proficient": False, "modifier": 3},
                {"name": "Perception", "stat": "WIS", "proficient": True, "modifier": 3},
                {"name": "Performance", "stat": "CHA", "proficient": False, "modifier": -1},
                {"name": "Persuasion", "stat": "CHA", "proficient": True, "modifier": 1},
                {"name": "Religion", "stat": "INT", "proficient": False, "modifier": 3},
                {"name": "Sleight of Hand", "stat": "DEX", "proficient": False, "modifier": 3},
                {"name": "Stealth", "stat": "DEX", "proficient": False, "modifier": 3},
                {"name": "Survival", "stat": "WIS", "proficient": False, "modifier": 1}
            ],
            "feats_traits": [
                "Human Traits: Lucky, Insightful",
                "Human ASI: +2 Dexterity, +1 Intelligence",
                "Background: Sage (Arcana, History)",
                "Feat: Druid Initiate (Guidance, Mending, Charm Person)",
                "Fighting Style: Two-Weapon Fighting",
                "Second Wind",
                "Weapon Masteries: Scimitar, Shortsword, Rapier"
            ],
            "spell_slots": {},
            "current_spell_slots": {},
            ...
            "prepared_spells": [],
            "auto_prepared_spells": ["Charm Person"]
        }
        ...
        return {
            ...
            "Mending": {
                ...
                "description": (
                    ...
                    "as the break or tear is no longer than 1 foot in any dimension, you mend it, leaving "
                    "no trace of the former damage. The spell can physically repair a magic item or construct, "
            ...
            "Booming Blade": {
                ...
                "description": (
                    ...
                    "Duration: 1 round\n\n"
                    "As part of the action used to cast this spell, you must make a melee attack with a "
                    "weapon against one creature within the spell's range, otherwise the spell fails. On a hit, "
                    "the target suffers the attack's normal effects, and it becomes sheathed in booming energy "
                    "until the start of your next turn. If the target willingly moves 5 feet or more before then, "
                    "it takes thunder damage and the spell ends."
                ),
                "spell_attack": False,
                "enhances_attack": True,
                "has_delayed_damage": True,
                "delayed_description": "If the target willingly moves before your next turn, it takes the listed thunder damage."
            },
            "Sword Burst": {
                "level": 0,
                "damage_dice": "1d6",
                ...
                "description": (
                    ...
                    "Components: V\n"
                    "Duration: Instantaneous\n\n"
                    "You create a momentary circle of spectral blades that sweep around you. All other creatures "
                    "within 5 feet of you must succeed on a Dexterity saving throw or take force damage."
                ),
                "spell_attack": False
            },
            "Mage Hand": {
                ...
                "description": (
                    ...
                    "an unlocked door or container, stow or retrieve an item from an open container, or pour the contents "
                    "out of a vial. You can move the hand up to 30 feet each time you use it. The hand can't attack, "
            ...
            "Green Flame Blade": {
                ...
                "description": (
                    ...
                    "suffers the attack's normal effects, and green fire leaps from the target to a different creature "
                    "of your choice that you can see within 5 feet of it. The second creature takes fire damage."
                ),
                "spell_attack": False,
                "enhances_attack": True
            },
            ...
            "Charm Person": {
                ...
                "description": (
                    ...
                    "Components: V, S\n"
                    "Duration: 1 hour\n\n"
                    "You attempt to charm a humanoid you can see within range. It must make a Wisdom saving throw, and "
                    "does so with advantage if you or your companions are fighting it. If it fails the saving throw, it "
                    "is charmed by you until the spell ends or until you or your companions do anything harmful to it. "
                    ...
                    "Druid Initiate: Cast once per long rest for free (no spell slot)."
                ),
                "spell_attack": False,
            ...
            "Magic Missile": {
                ...
                "description": (
                    ...
                    "can see within range. A dart deals force damage to its target. The darts all strike simultaneously, "
                    "and you can direct them to hit one creature or several. When you cast this spell using a spell slot of "
            ...
            "Sleep": {
                ...
                "description": (
                    ...
                    "Duration: Concentration, up to 1 minute\n\n"
                    "Each creature of your choice in a 5-foot-radius Sphere centered on a point within range must succeed on a "
                    "Wisdom saving throw or have the Incapacitated condition until the end of its next turn, at which point it "
                    "must repeat the save. If the target fails the second save, the target has the Unconscious condition for the "
                    "duration. The spell ends on a target if it takes damage or someone within 5 feet of it takes an action to "
                    "shake it out of the spell's effect.\n\n"
                    "Creatures that don't sleep, such as elves, or that have Immunity to the Exhaustion condition automatically "
            ...
            "Identify": {
                ...
                "description": (
                    ...
                    "If the item was created by a spell, you learn which spell created it. If you instead touch a creature throughout "
                    "the casting, you learn what spells, if any, are currently affecting it."
                )
            },
            "Prot E&G": {
                "level": 1,
                ...
                "description": (
                    ...
                    "Until the spell ends, one willing creature you touch is protected against aberrations, celestials, elementals, "
                    "fey, fiends, and undead. The protection grants several benefits: creatures of those types have disadvantage on "
                    "attack rolls against the target; the target also can't be charmed, frightened, or possessed by them. If the target "
                    "is already charmed, frightened, or possessed by such a creature, the target has advantage on any new saving throw "
            ...
            "Ice Knife": {
                ...
                "description": (
                    ...
                    "On a hit, the target takes piercing damage. Hit or miss, the shard then explodes. The target and each creature within "
                    "5 feet of it must succeed on a Dexterity saving throw or take cold damage."
                ),
                "spell_attack": True,
                "has_delayed_damage": True,
                "delayed_description": "Creatures within 5 feet make Dexterity saves or take the cold damage."
            },
            "Absorb Elements": {
                "level": 1,
                "damage_dice": "1d6",
                ...
                "description": (
                    ...
                    "you hit with a melee attack on your next turn, the target takes extra damage of the triggering type, and the spell ends."
                ),
                "spell_attack": False,
                "has_delayed_damage": True,
                "delayed_description": "Extra damage of the absorbed type is applied to your next melee attack before the spell ends."
            },
            "Detect Magic": {
                "level": 1,
                "damage_dice": "",
                ...
                "description": (
                    ...
                    "Components: V, S\n"
                    "Duration: Concentration, up to 10 minutes\n\n"
                    "For the duration, you sense the presence of magic within 30 feet of you. If you sense magic "
                    "in this way, you can use your action to see a faint aura around any visible creature or object "
                    "in the area that bears magic, and you learn its school of magic, if any.\n\n"
                    "The spell can penetrate most barriers, but is blocked by 1 foot of stone, 1 inch of common "
            ...
            "Shatter": {
                ...
                "description": (
                    ...
                    "Duration: Instantaneous\n\n"
                    "A sudden loud ringing noise, painfully intense, erupts from a point of your choice within range. Each creature "
                    "in a 10-foot-radius sphere centered on that point must make a Constitution saving throw. A creature takes thunder "
                    "damage on a failed save, or half as much on a successful one. A creature made of inorganic material such as stone, "
                    "crystal, or metal has disadvantage on this saving throw. A nonmagical object that isn't being worn or carried also "
                    "takes damage if it's in the spell's area."
                ),
                "spell_attack": False
            },
            "Shadow Blade": {
                ...
                "description": (
                    ...
                    "until the spell ends. It counts as a simple melee weapon with which you are proficient. It deals psychic damage on a hit "
                    "and has the finesse, light, and thrown properties (range 20/60 feet). In dim light or darkness you make attack rolls with advantage. "
            ...
            "Shield": {
                ...
                "description": (
                    ...
                    "to AC, including against the triggering attack, and you take no damage from magic missile."
                ),
            ...
            "Rime's Binding Ice": {
                ...
                "description": (
                    ...
                    "saving throw. On a failed save, a creature takes cold damage and is hindered by ice formations; its speed is reduced "
                    "to 0 until the end of your next turn. On a successful save, the creature takes half as much damage and isn't hindered. "
            ...
            "Warding Wind": {
                ...
                "description": (
                    ...
                    "The wind deafens you and other creatures in its area, extinguishes unprotected flames, hedges out vapor, gas, and fog, "
                    "and makes the area difficult terrain for creatures other than you. Attack rolls of ranged weapon attacks have disadvantage "
    ...
    def load_character_data(self):
        ...
        try:
            if os.path.exists(SAVE_FILE):
                ...
                self.bladesong_active = data.get("bladesong_active", False)
                self.bladesong_max_uses = data.get("bladesong_max_uses", self.proficiency_bonus)
                bladesong_uses = data.get("bladesong_uses_remaining", self.bladesong_max_uses)
                self.bladesong_uses_remaining = max(0, min(self.bladesong_max_uses, bladesong_uses))
                ...
                self.prepared_spells = data.get("prepared_spells", [])
                self.auto_prepared_spells = data.get("auto_prepared_spells", [])
                spell_data_updated = self.normalize_spell_records()
                self.max_prepared_spells = self.calculate_max_prepared_spells()
                # Normalize prepared spells ordering and limit
                normalized_prepared = []
                for spell in self.get_all_preparable_spells():
                    if spell in self.prepared_spells and spell not in normalized_prepared:
                        normalized_prepared.append(spell)
                self.prepared_spells = normalized_prepared[:self.max_prepared_spells] if self.max_prepared_spells else []
                ...
                if spell_data_updated:
                    logging.info("Normalized spell records (names, levels, preparation flags).")
                ...
                if "bladesong" not in self.actions and (self.bladesong_max_uses > 0 or "Wizard" in self.char_class):
                    self.actions["bladesong"] = {
                        "name": "Bladesong",
                        "description": "Bonus Action: Activate bladesong for 1 minute (PB uses/long rest)",
                        "available": self.bladesong_active or self.bladesong_uses_remaining > 0,
                        "recharge": "long_rest",
                        "uses": self.bladesong_uses_remaining,
                        "max_uses": self.bladesong_max_uses
                    }
                elif "bladesong" in self.actions:
                    self.actions["bladesong"]["uses"] = self.bladesong_uses_remaining
                    self.actions["bladesong"]["max_uses"] = self.bladesong_max_uses
                    self.actions["bladesong"]["available"] = self.bladesong_active or self.bladesong_uses_remaining > 0
                ...
                self.bladesong_active = default_data.get("bladesong_active", False)
                self.bladesong_uses_remaining = default_data.get("bladesong_uses_remaining", 0)
                self.bladesong_max_uses = default_data.get("bladesong_max_uses", 0)
                ...
                self.prepared_spells = default_data.get("prepared_spells", [])
                self.auto_prepared_spells = default_data.get("auto_prepared_spells", [])
                self.normalize_spell_records()
                self.max_prepared_spells = self.calculate_max_prepared_spells()
                self.weapons = default_data["weapons"]
                self.fighting_styles = default_data["fighting_styles"]
    ...
    def save_character_data(self):
        ...
        try:
            ...
            data = {
                ...
                "bladesong_active": self.bladesong_active,
                "bladesong_uses_remaining": self.bladesong_uses_remaining,
                "bladesong_max_uses": self.bladesong_max_uses,
                ...
                "auto_prepared_spells": self.auto_prepared_spells,
    ...
    def toggle_inspiration(self, increment=True):
        ...
        if increment:
            self.inspiration = min(2, self.inspiration + 1)
    ...
    def toggle_shield(self):
        """Deactivate Shield spell AC bonus (only deactivates, doesn't activate)"""
        if self.shield_active:
            # Deactivate Shield - return to base AC
            self.shield_active = False
            logging.info(f"Shield spell ended - AC returned to {self.ac}")
        else:
            logging.info("Shield is not currently active - cast Shield spell to activate it")
        self.recalculate_ac()
        # Auto-save after AC change
        self.save_character_data()

    def toggle_mage_armor(self):
        """Toggle Mage Armor effect."""
        if self.mage_armor_active:
            self.mage_armor_active = False
            logging.info("Mage Armor dismissed. Base AC restored to worn armor.")
        else:
            self.mage_armor_active = True
            logging.info("Mage Armor activated. Base AC set to 13 + DEX modifier.")
        self.recalculate_ac()
        self.save_character_data()

    def recalculate_ac(self):
        """Recalculate AC based on base, Shield, and Bladesong bonuses."""
        dex_modifier = self.stats.get("DEX", {}).get("modifier", 0)
        base_ac = self.base_ac if self.base_ac is not None else 10
        if self.mage_armor_active:
            mage_base_ac = 13 + dex_modifier
            base_ac = mage_base_ac
        self.current_base_ac = base_ac

        shield_bonus = 5 if self.shield_active else 0
        int_modifier = self.stats.get("INT", {}).get("modifier", 0)
        bladesong_bonus = max(1, int_modifier) if self.bladesong_active else 0
        self.ac = base_ac + shield_bonus + bladesong_bonus

    def get_class_level(self, class_name: str) -> int:
        """Extract total level for a specific class from the char_class string."""
        total = 0
        class_name = class_name.lower()
        for part in self.char_class.split('/'):
            segment = part.strip()
            if not segment:
                continue
            ...
            if not match:
                continue
            level_token = match.group(1)
            cls_name = segment[:match.start()].split('(')[0].strip().lower()
            if cls_name == class_name:
                try:
                    total += int(level_token)
                except ValueError:
                    continue
        return total

    def calculate_max_prepared_spells(self) -> int:
        """Calculate maximum prepared spells based on wizard level and INT modifier."""
        wizard_level = self.get_class_level("wizard")
        int_modifier = self.stats.get("INT", {}).get("modifier", 0)
        if wizard_level <= 0:
            return 0
        return max(1, wizard_level + int_modifier)

    def get_all_preparable_spells(self):
        """Return ordered list of preparable spells (leveled, excluding auto-prepared)."""
        preparable = []
        for level_key in sorted(self.spells.keys(), key=self.sort_spell_level_key):
            if level_key == "0" or level_key.lower() == "cantrip":
                continue
            for spell in self.spells.get(level_key, []):
                if spell not in self.auto_prepared_spells:
                    preparable.append(spell)
        return preparable

    @staticmethod
    def sort_spell_level_key(level_key: str) -> int:
        """Convert spell level key to numeric sorting value."""
        mapping = {
            "cantrip": 0,
            "0": 0,
            "1st": 1,
        ...
        return mapping.get(level_key.lower(), 99)

    def _resolve_spell_level(self, spell_name):
        """Look up the actual level for a spell from spell_data."""
        info = self.spell_data.get(spell_name)
        if info:
            return info["level"]
        return 1

    def get_all_spells_ordered(self):
        """Return all spells in the current sorted/render order."""
        spells = []
        seen = set()
        for cantrip in self.cantrips:
            spells.append((cantrip, 0))
            seen.add(cantrip)
        for spell in self.prepared_spells:
            if spell not in seen:
                spells.append((spell, self._resolve_spell_level(spell)))
                seen.add(spell)
        for spell in self.auto_prepared_spells:
            if spell not in seen:
                spells.append((spell, self._resolve_spell_level(spell)))
                seen.add(spell)
        for spell in self.get_all_preparable_spells():
            if spell not in seen:
                spells.append((spell, self._resolve_spell_level(spell)))
                seen.add(spell)
        other_levels = [key for key in sorted(self.spells.keys(), key=self.sort_spell_level_key) if key not in ["0", "cantrip", "1st"]]
        for level_key in other_levels:
            numeric_level = self.sort_spell_level_key(level_key)
            for spell in self.spells.get(level_key, []):
                if spell not in seen:
                    spells.append((spell, numeric_level))
                    seen.add(spell)
        return spells
    
    def normalize_spell_records(self) -> bool:
        """Normalize spell naming, level placement, and preparation flags."""
        updated = False

        rename_map = {
            "Protection from Evil and Good": "Prot E&G"
        }

        if self.spells is None:
            self.spells = {}

        # Rename spells across spell lists
        for old_name, new_name in rename_map.items():
            if old_name == new_name:
                continue

            for level_key, spell_list in list(self.spells.items()):
                if not spell_list:
                    continue
                new_list = [new_name if spell == old_name else spell for spell in spell_list]
                if new_list != spell_list:
                    self.spells[level_key] = new_list
                    updated = True

            if old_name in self.prepared_spells:
                self.prepared_spells = [new_name if spell == old_name else spell for spell in self.prepared_spells]
                updated = True

            if old_name in self.auto_prepared_spells:
                self.auto_prepared_spells = [new_name if spell == old_name else spell for spell in self.auto_prepared_spells]
                updated = True

        # Ensure Charm Person is treated as auto-prepared via Druid Initiate
        if "Charm Person" not in self.auto_prepared_spells:
            self.auto_prepared_spells.append("Charm Person")
            updated = True

        # Remove Charm Person from prepared list if present (it shouldn't count against limit)
        if "Charm Person" in self.prepared_spells:
            self.prepared_spells = [spell for spell in self.prepared_spells if spell != "Charm Person"]
            updated = True

        # Prevent duplicates of higher-level spells at 1st level
        first_level_spells = self.spells.get("1st", [])
        if first_level_spells:
            filtered = [spell for spell in first_level_spells if spell not in {"Misty Step", "Shadow Blade"}]
            if len(filtered) != len(first_level_spells):
                self.spells["1st"] = filtered
                updated = True

        # Ensure spell lists don't contain duplicate entries
        for level_key, spell_list in list(self.spells.items()):
            if not spell_list:
                continue
            deduped = list(dict.fromkeys(spell_list))
            if deduped != spell_list:
                self.spells[level_key] = deduped
                updated = True

        # Ensure auto/prepared lists are deduplicated
        self.auto_prepared_spells = list(dict.fromkeys(self.auto_prepared_spells))
        self.prepared_spells = list(dict.fromkeys(self.prepared_spells))

        return updated

    def build_spell_render_list(self):
        """Compatibility helper for spell list rendering."""
        return self.get_all_spells_ordered()

    def toggle_bladesong(self):
        """Activate or deactivate Bladesong if uses remain."""
        int_modifier = self.stats.get("INT", {}).get("modifier", 0)
        bladesong_bonus = max(1, int_modifier)

        if self.bladesong_active:
            self.bladesong_active = False
            logging.info("Bladesong dismissed. Bonuses removed.")
        else:
            if self.bladesong_uses_remaining <= 0:
                logging.warning("No Bladesong uses remaining! Take a long rest to recharge.")
                return False
            self.bladesong_active = True
            self.bladesong_uses_remaining -= 1
            logging.info(f"Bladesong activated! AC +{bladesong_bonus}, speed +10 ft, ADV Acrobatics, INT for attacks, +{bladesong_bonus} CON concentration saves.")
    ...
    def short_rest(self):
        """Perform a short rest - restore some resources"""
        # Restore half HP (rounded up)
        hp_restored = (self.max_hp - self.current_hp) // 2
        if hp_restored > 0:
            self.current_hp = min(self.max_hp, self.current_hp + hp_restored)
            logging.info(f"Short rest: restored {hp_restored} HP. Current HP: {self.current_hp}/{self.max_hp}")
        else:
            logging.info("Short rest: already at full health!")
        
        # Restore actions that recharge on short rest
        actions_recharged = []
    ...
    def long_rest(self):
        ...
        self.luck_points = 3
        # Reset Bladesong
        old_bladesong_uses = self.bladesong_uses_remaining
        self.bladesong_uses_remaining = self.bladesong_max_uses
        if self.bladesong_active:
            self.bladesong_active = False
        if "bladesong" in self.actions:
            self.actions["bladesong"]["uses"] = self.bladesong_uses_remaining
            self.actions["bladesong"]["available"] = True
        ...
        # Dismiss Shadow Blade concentration
        self.shadow_blade_active = False
        ...
        logging.info(f"- Reset Druid Initiate free Charm Person cast")
        logging.info(f"- Gained +1 inspiration (Insightful): {old_inspiration} → {self.inspiration}")
        logging.info(f"- Reset Luck points to 3")
        logging.info(f"- Bladesong uses refreshed ({old_bladesong_uses} → {self.bladesong_uses_remaining})")
        if actions_recharged:
            logging.info(f"- Recharged actions: {', '.join(actions_recharged)}")
    ...
    def get_effective_attack_stat(self, weapon):
        """Return the effective stat key and modifier for a weapon, accounting for Bladesong Bladework."""
        base_stat = weapon["attack_stat"]
        base_mod = self.stats[base_stat]["modifier"]
        if self.bladesong_active:
            int_mod = self.stats.get("INT", {}).get("modifier", 0)
            if int_mod > base_mod:
                return "INT", int_mod
        return base_stat, base_mod

    def calculate_attack_bonus(self, weapon):
        """Calculate attack bonus for a weapon"""
        _, stat_modifier = self.get_effective_attack_stat(weapon)
    ...
    def get_attack_weapons(self):
        """Return the weapon list used by the attack menu, including Shadow Blade when active."""
        weapons = list(self.weapons)
        if self.shadow_blade_active:
            weapons.insert(0, self.shadow_blade_weapon)
        return weapons

    def create_attack_buttons(self):
        """Create weapon selection buttons for attack menu"""
        self.attack_buttons = []
        self._attack_weapon_list = self.get_attack_weapons()

        for i, weapon in enumerate(self._attack_weapon_list):
            if weapon["name"] == "Shadow Blade":
                color = (90, 50, 120)
            elif "Broadsword" in weapon["name"]:
                color = DANGER
            else:
                color = (100, 80, 60)
    ...
    def create_spell_buttons(self):
        ...
        render_list = self.build_spell_render_list()
        total_spells = len(render_list)
        ...
        visible_spells = render_list[self.spell_scroll_offset:self.spell_scroll_offset + self.max_visible_spells]
        
        prepared_set = set(self.prepared_spells)
        auto_set = set(self.auto_prepared_spells)
        ...
        for idx, (spell_name, level) in enumerate(visible_spells):
            if level == 0:
                ...
                color = (60, 80, 100) if level == 1 else (100, 60, 80)  # Blue for 1st, red for 2nd
                suffix = {1: "st", 2: "nd", 3: "rd"}.get(level, "th")
                display_name = f"{spell_name} ({level}{suffix} Level)"
                if spell_name in auto_set:
                    aux_toggle = {"label": "AUTO", "state": True, "locked": True}
                else:
                    aux_toggle = {"label": "PREP", "state": spell_name in self.prepared_spells}
            ...
            button = ModernButton(
                ...
                has_advantage_checkbox=False,
                aux_toggle=aux_toggle
            )
            button.spell_name = spell_name
            button.spell_level = level
            button.is_auto_prepared = spell_name in self.auto_prepared_spells
    ...
    def create_action_buttons(self):
        ...
        self.action_buttons = []
        self.action_action_buttons = []
        self.current_action_enabled = True
        
        if "bladesong" in self.actions:
            bladesong_data = self.actions["bladesong"]
            bladesong_data["uses"] = self.bladesong_uses_remaining
            bladesong_data["max_uses"] = self.bladesong_max_uses
            bladesong_data["available"] = self.bladesong_active or self.bladesong_uses_remaining > 0
        ...
        for i, action_key in enumerate(actions):
            ...
            if action_key == "bladesong":
                if self.bladesong_active:
                    color = (90, 130, 160)  # Blue tint when active
                elif self.bladesong_uses_remaining > 0:
                    color = (80, 120, 80)  # Green for uses remaining
                else:
                    color = (100, 100, 100)
            else:
                color = (80, 120, 80) if action_data.get("available", True) else (100, 100, 100)
                
            button_text = action_data["name"]
        ...
        if self.selected_action:
            ...
            if self.selected_action == "bladesong":
                button_label = "DEACTIVATE" if self.bladesong_active else "ACTIVATE"
                action_available = self.bladesong_active or self.bladesong_uses_remaining > 0
                button_color = (150, 100, 170) if not self.bladesong_active else (120, 140, 200)
                if not action_available and not self.bladesong_active:
                    button_color = (100, 100, 100)
            else:
                button_label = "USE ACTION"
                button_color = (120, 80, 150)
            
            # Center the button below the info box (info box ends at y=450)
            button_x = (self.current_width - 200) // 2  # Center horizontally
            button_y = 470  # Below the info box
            ...
            self.current_action_enabled = action_available or self.bladesong_active
    ...
    def get_sorted_sessions(self):
        """Return session names sorted by the Date: DD.MM.YY found in their content (oldest first).
        Entries without a parseable date are placed at the beginning."""
        sessions_dict = self.journal.get("sessions", {})
        date_pattern = re.compile(r"Date:\s*(\d{2})\.(\d{2})\.(\d{2})")

        def _sort_key(name):
            match = date_pattern.search(sessions_dict.get(name, ""))
            if match:
                day, month, year = int(match.group(1)), int(match.group(2)), int(match.group(3))
                return (1, year, month, day)
            return (0,)

        return sorted(sessions_dict.keys(), key=_sort_key)
    ...
    def create_journal_buttons(self):
        ...
        sessions = self.get_sorted_sessions()
    ...
    def handle_spell_preparation_toggle(self, button: ModernButton):
        """Toggle prepared state for a spell button."""
        spell_name = getattr(button, "spell_name", None)
        if not spell_name or getattr(button, "is_auto_prepared", False):
            button.aux_toggle_state = True
            return
        
        desired_state = button.aux_toggle_state
        changed = False
        if desired_state:
            if spell_name not in self.prepared_spells:
                if self.max_prepared_spells and len(self.prepared_spells) >= self.max_prepared_spells:
                    button.aux_toggle_state = False
                    logging.warning("Spell preparation limit reached.")
                    self.temp_overlay_text = f"Prep limit reached ({self.max_prepared_spells})"
        ...
        if changed:
            ordered = []
            for spell in self.get_all_preparable_spells():
                if spell in self.prepared_spells and spell not in ordered:
                    ordered.append(spell)
            self.prepared_spells = ordered
            logging.info(f"Prepared spells updated: {self.prepared_spells}")
            self.save_character_data()
            self.create_spell_buttons()
    ...
    def draw_save_info_box(self, screen):
        ...
        bladesong_bonus = 0
        if self.bladesong_active and self.selected_save == "CON":
            bladesong_bonus = max(1, self.stats.get("INT", {}).get("modifier", 0))
            total_bonus += bladesong_bonus
        ...
        if bladesong_bonus:
            breakdown += f" + BS {bladesong_bonus:+d}"
        breakdown += ")"
        ...
        desc_lines = self.wrap_text(desc, self.small_font, width - 40)
        desc_y = card_y + 110
        for line in desc_lines:
            desc_surface = self.small_font.render(line, True, TEXT_SECONDARY)
            screen.blit(desc_surface, (x + 20, desc_y))
            desc_y += 18
    ...
    def draw_spell_info_box(self, screen):
        ...
        level_text = "Cantrip" if spell_info["level"] == 0 else f"Level {spell_info['level']} Spell"
        level_surface = self.text_font.render(level_text, True, TEXT_PRIMARY)
        screen.blit(level_surface, (x + 20, card_y + 20))
        
        # Spell description (handle line breaks)
        desc_y = card_y + 55
        max_desc_width = width - 60
        description_lines = self.wrap_text_paragraph(spell_info["description"], self.small_font, max_desc_width)
        ...
        # Damage info (if applicable) - position after description
        if spell_info.get("has_damage", False):
            damage_y = info_bottom + 10
            damage_text = f"Damage: {spell_info['damage_dice']} {spell_info['damage_type']}"
            damage_surface = self.text_font.render(damage_text, True, TEXT_PRIMARY)
            screen.blit(damage_surface, (x + 20, damage_y))
            info_bottom = damage_y + self.text_font.get_height()
            
            # Spell attack modifier (for Eldritch Knight)
            spell_mod = self.stats["INT"]["modifier"]
            spell_attack_bonus = spell_mod + self.proficiency_bonus
            
            if spell_info.get("spell_attack", True):
                attack_y = info_bottom + 5
                attack_text = f"Spell Attack: 1d20 + {spell_attack_bonus} (INT {spell_mod:+d} + Prof {self.proficiency_bonus:+d})"
                attack_surface = self.small_font.render(attack_text, True, TEXT_SECONDARY)
                screen.blit(attack_surface, (x + 20, attack_y))
                info_bottom = attack_y + self.small_font.get_height()
                
                dc_y = info_bottom + 5
                spell_dc_text = f"Spell Save DC: {8 + spell_attack_bonus}"
                dc_surface = self.small_font.render(spell_dc_text, True, TEXT_SECONDARY)
                screen.blit(dc_surface, (x + 20, dc_y))
                info_bottom = dc_y + self.small_font.get_height()
            else:
                dc_y = info_bottom + 5
                spell_dc_text = f"Spell Save DC: {8 + spell_attack_bonus} (targets make saves)"
                dc_surface = self.small_font.render(spell_dc_text, True, TEXT_SECONDARY)
                screen.blit(dc_surface, (x + 20, dc_y))
                info_bottom = dc_y + self.small_font.get_height()
        
        # Position spell action buttons below the info box with a small margin
        if self.spell_action_buttons:
            info_box_center = x + width // 2
            cast_button_width = 169
            button_spacing = 20
            button_y = y + height + 15

            cast_button_x = info_box_center - cast_button_width - button_spacing // 2
            self.spell_action_buttons[0].rect.x = cast_button_x
            self.spell_action_buttons[0].rect.y = button_y

            delayed_button_x = info_box_center + button_spacing // 2
            self.spell_action_buttons[1].rect.x = delayed_button_x
            self.spell_action_buttons[1].rect.y = button_y
    ...
    def draw_weapon_info_box(self, screen):
        ...
        eff_stat, stat_mod = self.get_effective_attack_stat(self.selected_weapon)
        stat_label = f"{eff_stat} (Bladesong)" if eff_stat != self.selected_weapon["attack_stat"] else eff_stat
        breakdown = f"({stat_label} {stat_mod:+d} + Prof {self.proficiency_bonus:+d}"
        ...
        damage_breakdown = f"({stat_label} {stat_mod:+d}"
        if self.selected_weapon.get("magic_bonus", 0):
    ...
    def draw_spell_slots(self, screen):
        """Draw spell slot tracking on the right side"""
        x = self.current_width - 220
        y = 150
        
        # Spell slots card
        card_y = self.draw_card(screen, x, y, 200, 250, "Spell Slots", "center", 200)
        
        current_y = card_y + 20
        
        for level in ["1st", "2nd"]:
            if level in self.spell_slots:
                max_slots = self.spell_slots[level]
                # Get current remaining slots
                ...
                for i in range(max_slots):
                    ...
                    # Draw slot circle (filled if available, empty if used) - warm sunrise orange
                    slot_color = (255, 165, 80) if i < remaining_slots else (60, 60, 60)  # Orange vs dark gray
    ...
    def draw_health_display(self, screen):
        ...
        # Calculate spacing for elements: [LUCK][INSP] gap [HP BAR] gap [AC]
        luck_w, insp_w, bar_w, ac_w = 50, 50, 240, 60
        small_gap = 25  # Small gap between grouped elements and HP bar
        
        # Total width of all elements
        total_width = luck_w + insp_w + small_gap + bar_w + small_gap + ac_w
        
        # Start position to center the group
        start_x = card_x + (card_width - total_width) // 2 - luck_w / 2
        
        # Position elements
        luck_x = start_x
        insp_x = luck_x + luck_w + small_gap/2  # INSP right next to LUCK
        bar_x_adjusted = insp_x + insp_w + small_gap  # Small gap before HP bar
        ac_x = bar_x_adjusted + bar_w + small_gap  # Small gap after HP bar
        
        # HP Bar - bigger size, positioned with the group
        hp_percentage = self.current_hp / self.max_hp
        bar_width = 240
        bar_height = 16
        bar_x = bar_x_adjusted  # Use calculated position
        ...
        # Luck indicator (leftmost, aligned with HP bar) - square with number
        luck_y = card_y + 31  # Aligned with HP bar
        luck_width, luck_height = 50, 50
        self.luck_rect = pygame.Rect(luck_x, luck_y, luck_width, luck_height)  # Clickable area
        
        # Background color based on luck count
        if self.luck_points > 0:
            luck_bg_color = (255, 165, 80, 100)  # Faded orange
            luck_border_color = (255, 165, 80)  # Solid orange border
            luck_text_color = TEXT_PRIMARY
        else:
            luck_bg_color = (60, 60, 60, 100)  # Grayed out
            luck_border_color = (60, 60, 60)  # Gray border
            luck_text_color = TEXT_SECONDARY
        
        # Draw background
        luck_surface = pygame.Surface((luck_width, luck_height), pygame.SRCALPHA)
        pygame.draw.rect(luck_surface, luck_bg_color, (0, 0, luck_width, luck_height), border_radius=8)
        screen.blit(luck_surface, (luck_x, luck_y))
        
        # Draw border
        pygame.draw.rect(screen, luck_border_color, self.luck_rect, 2, border_radius=8)
        
        # Draw luck count
        luck_count_text = self.header_font.render(str(self.luck_points), True, luck_text_color)
        luck_count_rect = luck_count_text.get_rect(center=(luck_x + luck_width // 2, luck_y + luck_height // 2 - 5))
        screen.blit(luck_count_text, luck_count_rect)
        
        # Draw label
        luck_label = self.small_font.render("LUCK", True, luck_text_color)
        luck_label_rect = luck_label.get_rect(center=(luck_x + luck_width // 2, luck_y + luck_height - 10))
        screen.blit(luck_label, luck_label_rect)
        
        # Inspiration indicator (second from left, aligned with HP bar) - square with number
        ...
        # AC (rightmost, aligned with HP bar) using UI box - clickable for Shield spell
        ...
        if self.ui_box_image:
            ...
            pygame.draw.rect(ac_surface, (60, 60, 70, 180), (0, 0, ac_width, ac_height), border_radius=8)
            screen.blit(ac_surface, (ac_x, ac_y))
        
        if self.shield_active:
            shield_surface = pygame.Surface((ac_width, ac_height), pygame.SRCALPHA)
            pygame.draw.rect(shield_surface, (255, 165, 80, 60), (0, 0, ac_width, ac_height), border_radius=8)
            screen.blit(shield_surface, (ac_x, ac_y))
        
        if self.bladesong_active:
            bladesong_surface = pygame.Surface((ac_width, ac_height), pygame.SRCALPHA)
            pygame.draw.rect(bladesong_surface, (90, 160, 200, 50), (0, 0, ac_width, ac_height), border_radius=8)
            screen.blit(bladesong_surface, (ac_x, ac_y))
        ...
        indicator_width = ac_width
        indicator_height = ac_height
        indicator_spacing = 10
        indicator_x = ac_x + ac_width + indicator_spacing
        ...
        # Bladesong indicator
        bladesong_bonus = max(1, self.stats.get("INT", {}).get("modifier", 0))
        bladesong_subtitle = f"+{bladesong_bonus} AC" if self.bladesong_active else (
            f"{self.bladesong_uses_remaining}/{self.bladesong_max_uses} uses" if self.bladesong_max_uses else "Unavailable"
        )
        self.bladesong_indicator_rect = pygame.Rect(indicator_x, ac_y, indicator_width, indicator_height)
        draw_indicator(
            self.bladesong_indicator_rect,
            "BS",
            bladesong_subtitle,
            self.bladesong_active,
            (90, 160, 200, 80)
        )
        
        # Mage Armor indicator
        indicator_x += indicator_width + indicator_spacing
        mage_base_ac = 13 + self.stats.get("DEX", {}).get("modifier", 0)
        mage_subtitle = f"Base {self.current_base_ac}" if self.mage_armor_active else f"13+DEX ({mage_base_ac})"
        self.mage_armor_indicator_rect = pygame.Rect(indicator_x, ac_y, indicator_width, indicator_height)
        draw_indicator(
    ...
    def apply_hp_change(self):
        ...
        try:
            ...
            if actual_change > 0:
                logging.info(f"Barian healed {actual_change} HP ({old_hp} → {self.current_hp})")
            elif actual_change < 0:
                logging.info(f"Barian took {abs(actual_change)} damage ({old_hp} → {self.current_hp})")
    ...
    def draw_stats_section(self, screen):
        ...
        # Passive Perception below the ability score grid
        perception_mod = 0
        for skill in self.skills:
            if skill[0] == "Perception":
                perception_mod = skill[3]
                break
        passive_perception = 10 + perception_mod
        pp_y = start_y + 2 * 120 + 10
        right_edge = start_x + 2 * 80 + 50
        pp_value = self.header_font.render(str(passive_perception), True, TEXT_PRIMARY)
        pp_value_rect = pp_value.get_rect(midright=(right_edge, pp_y + self.small_font.get_height() // 2))
        screen.blit(pp_value, pp_value_rect)
        pp_label = self.small_font.render("Passive Perception", True, TEXT_SECONDARY)
        pp_label_rect = pp_label.get_rect(midright=(pp_value_rect.left - 8, pp_y + self.small_font.get_height() // 2))
        screen.blit(pp_label, pp_label_rect)

    def _wrap_text(self, text, font, max_width):
        """Word-wrap *text* to fit within *max_width* pixels. Returns a list of lines."""
        words = text.split()
        lines = []
        current = ""
        for word in words:
    ...
    def draw_feats_traits_section(self, screen):
        """Draw feats and traits section at the bottom with word wrap and hover-scroll."""
        left_boundary = 320
        right_boundary = self.current_width - 320
        available_width = right_boundary - left_boundary
        width = available_width - 40
        side_padding = (available_width - width) // 2
        x = left_boundary + side_padding
        y = 720
        height = 160

        card_y = self.draw_card(screen, x, y, width, height, "FEATS & TRAITS", "left")

        if not hasattr(self, "feats_scroll_offset"):
            self.feats_scroll_offset = 0

        content_x = x + 20
        content_width = width - 40
        content_height = height - (card_y - y) - 10
        line_height = self.small_font.get_linesize()

        wrapped_lines = []
        for feat in self.feats_traits:
            lines = self._wrap_text(feat, self.small_font, content_width - 15)
            wrapped_lines.append(lines)

        total_content_height = sum(len(lines) * line_height + 4 for lines in wrapped_lines)
        max_scroll = max(0, total_content_height - content_height)
        self.feats_scroll_offset = max(0, min(self.feats_scroll_offset, max_scroll))

        self.feats_rect = pygame.Rect(x, card_y, width, content_height)

        clip_rect = pygame.Rect(content_x, card_y, content_width, content_height)
        screen.set_clip(clip_rect)

        draw_y = card_y - self.feats_scroll_offset
        for lines in wrapped_lines:
            bullet = self.small_font.render("•", True, ACCENT)
            screen.blit(bullet, (content_x, draw_y))
            for j, line in enumerate(lines):
                line_surface = self.small_font.render(line, True, TEXT_PRIMARY)
                screen.blit(line_surface, (content_x + 15, draw_y + j * line_height))
            draw_y += len(lines) * line_height + 4

        screen.set_clip(None)

        if max_scroll > 0:
            mouse_pos = pygame.mouse.get_pos()
            if self.feats_rect.collidepoint(mouse_pos):
                hint = self.small_font.render("scroll", True, TEXT_SECONDARY)
                screen.blit(hint, (x + width - 55, y + 15))
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if self.hp_input_active and self.current_screen == "main":
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    # Allow text input for journal content
            ...
            if self.current_screen == "spells" and hasattr(event, "button"):
                if event.button == 4:  # Scroll up
                    ...
                    total_spells = len(self.get_all_spells_ordered())
                    max_offset = max(0, total_spells - self.max_visible_spells)
            ...
            if self.current_screen == "journal" and hasattr(event, "button"):
                if event.button == 4:  # Scroll up
                    ...
                    self.scroll_journal(1)
                    return
            if self.current_screen == "main" and hasattr(self, "feats_rect") and hasattr(event, "button"):
                if self.feats_rect.collidepoint(event.pos):
                    scroll_step = 15
                    if event.button == 4:
                        self.feats_scroll_offset = max(0, self.feats_scroll_offset - scroll_step)
                        return
                    elif event.button == 5:
                        self.feats_scroll_offset += scroll_step
                        return
            ...
            # Check if luck was clicked (only on main screen)
            if (self.current_screen == "main" and 
                hasattr(self, 'luck_rect') and 
                self.luck_rect.collidepoint(event.pos)):
                # Left-click to increment, right-click to decrement
                if event.button == 1:  # Left-click
                    self.toggle_luck(increment=True)
                elif event.button == 3:  # Right-click
                    self.toggle_luck(increment=False)
                return
            ...
            # Check Bladesong indicator
            if (self.current_screen == "main" and 
                getattr(self, 'bladesong_indicator_rect', None) and 
                self.bladesong_indicator_rect.collidepoint(event.pos)):
                if event.button == 1:
                    self.toggle_bladesong()
                return
            
            # Check Mage Armor indicator
            if (self.current_screen == "main" and 
            ...
            if self.current_screen == "spells":
                ...
                self.create_spell_buttons()
                return
            if self.current_screen == "journal":
                if event.y > 0:
                    self.scroll_journal(-1)
                elif event.y < 0:
                    self.scroll_journal(1)
                return
            if self.current_screen == "main" and hasattr(self, "feats_rect"):
                mouse_pos = pygame.mouse.get_pos()
                if self.feats_rect.collidepoint(mouse_pos):
                    scroll_step = 15
                    if event.y > 0:
        ...
        for i, button in enumerate(self.attack_buttons):
            ...
            if click_type == 'button' and clicked:
                if i == len(self.attack_buttons) - 1:  # Back button
                    ...
                    self.last_attack_roll = None
                    self.last_damage_roll = None
                    self.selected_weapon = self._attack_weapon_list[i]
    ...
    def handle_spell_menu_clicks(self, pos):
        ...
        for i, button in enumerate(self.spell_buttons):
            ...
            if click_type == 'toggle':
                if hasattr(button, "spell_name"):
                    self.handle_spell_preparation_toggle(button)
                return
            if click_type == 'toggle_locked':
                logging.info("This spell is always prepared.")
                return
            if click_type == 'button' and clicked:
    ...
    def handle_spell_button_press(self, button_index):
        ...
        if button_index < len(self.spell_buttons):
            ...
            if button_index == len(self.spell_buttons) - 1:  # Back button
                ...
                # Find the selected spell from our spell list
                all_spells = []
                for cantrip in self.cantrips:
                    all_spells.append(cantrip)
                for spell in self.spells.get("1st", []):
                    all_spells.append(spell)
                for spell in self.spells.get("2nd", []):
                    all_spells.append(spell)
                
                if button_index < len(all_spells):
                    self.selected_spell = all_spells[button_index]
                    logging.info(f"Selected spell: {self.selected_spell}")
    ...
    def perform_attack_roll(self):
        ...
        has_advantage = self.attack_action_buttons[0].aux_toggle_state if len(self.attack_action_buttons) > 0 else False
    ...
    def perform_spell_damage_roll(self):
        ...
        # Check if this spell enhances attacks (like Booming Blade)
        if spell_info.get("enhances_attack", False):
            ...
            # Consume spell slot first if it's not a cantrip
            if spell_info["level"] > 0:
                level_key = f"{spell_info['level']}{'st' if spell_info['level'] == 1 else 'nd'}"
                if level_key in self.current_spell_slots and self.current_spell_slots[level_key] > 0:
                    self.current_spell_slots[level_key] -= 1
                    logging.info(f"Used {level_key} level spell slot. Remaining: {self.current_spell_slots[level_key]}")
                    self.save_character_data()
                else:
                    logging.warning(f"No {level_key} level spell slots remaining!")
                    return

            if self.selected_spell == "Shadow Blade":
                self.shadow_blade_active = True
                logging.info("Shadow Blade conjured! It appears in the attack menu.")
        ...
        if spell_info.get("has_damage", False):
            ...
            # Non-damage spell (like Shield, Misty Step)
            logging.info(f"Cast {self.selected_spell} - {spell_info['description']}")
            self.last_spell_damage_roll = None  # Clear any previous damage roll
            
            # Automatically activate Shield AC if this is the Shield spell
            if spell_info.get("activates_shield", False):
                self.shield_active = True
                self.recalculate_ac()
                logging.info(f"Shield spell activated - AC increased to {self.ac}")
            if spell_info.get("activates_mage_armor", False):
                self.mage_armor_active = True
                self.recalculate_ac()
                logging.info(f"Mage Armor activated - Base AC is now {self.current_base_ac}")
        
        # Consume spell slot if it's not a cantrip
        if spell_info["level"] > 0:
            # Check if this is Druid Initiate Charm Person with free cast available
            if spell_info.get("druid_free_cast", False) and not self.druid_charm_person_used:
                self.druid_charm_person_used = True
                logging.info(f"Cast {self.selected_spell} (Druid Initiate - free cast, no slot consumed)")
                # Auto-save after using free cast
                self.save_character_data()
            else:
                # Normal spell slot consumption
                level_key = f"{spell_info['level']}{'st' if spell_info['level'] == 1 else 'nd'}"
                if level_key in self.current_spell_slots and self.current_spell_slots[level_key] > 0:
                    self.current_spell_slots[level_key] -= 1
                    logging.info(f"Used {level_key} level spell slot. Remaining: {self.current_spell_slots[level_key]}")
                    # Auto-save after spell slot change
                    self.save_character_data()
                else:
                    logging.warning(f"No {level_key} level spell slots remaining!")
    ...
    def perform_save_roll(self):
        ...
        # Check if advantage is enabled
        has_advantage = self.saves_action_buttons[0].aux_toggle_state if len(self.saves_action_buttons) > 0 else False
    ...
    def build_journal_display_lines(self, text, wrapped_lines):
        """Build metadata mapping wrapped display lines to original text indices."""
        if text is None:
            text = ""
        metadata = []
        consumed_index = 0
        ...
        for line in wrapped_lines:
            if not text:
                metadata.append({"text": line, "start": 0, "end": 0})
                continue

            if line == "":
                start_idx = min(consumed_index, total_length)
                metadata.append({"text": line, "start": start_idx, "end": start_idx})
                if consumed_index < total_length and text[consumed_index] == '\n':
                    consumed_index += 1
                continue

            remaining_text = text[consumed_index:]
            relative_pos = remaining_text.find(line)
            if relative_pos == -1:
                relative_pos = 0
            start_idx = consumed_index + relative_pos
            start_idx = max(0, min(start_idx, total_length))
            end_idx = max(start_idx, min(start_idx + len(line), total_length))
            metadata.append({"text": line, "start": start_idx, "end": end_idx})
            consumed_index = end_idx
    ...
    def draw_journal_content(self, screen):
        ...
        if self.session_name_editing and not is_character_entry:
            # Draw editing background for session title
            ...
            # Draw cursor for session name editing
        ...
        wrapped_lines = self.wrap_text_paragraph(current_text, self.text_font, max_text_width)
        self.journal_display_lines = self.build_journal_display_lines(current_text, wrapped_lines)
        ...
        for i in range(start_line, end_line):
            line_surface = self.text_font.render(wrapped_lines[i], True, TEXT_PRIMARY)
            screen.blit(line_surface, (self.journal_content_rect.x, current_y))
            current_y += line_height
        
        # Draw cursor if text is active
        if self.journal_text_active:
            ...
            for i, meta in enumerate(display_lines):
                ...
                if start_idx <= cursor_pos <= end_idx:
                    cursor_line = i
                    cursor_offset = cursor_pos - start_idx
                    break
                if cursor_pos > end_idx:
                    cursor_line = i
                    cursor_offset = meta.get("end", start_idx) - start_idx
            ...
            if start_line <= cursor_line < end_line:
                ...
                cursor_offset = max(0, min(cursor_offset, len(line_text)))
                line_before_cursor = line_text[:cursor_offset]
        ...
        # Draw instruction text if area is active (moved up due to taller box)
        if self.journal_text_active:
            ...
            # Show click instruction when not active
            if is_character_entry:
        ...
        # Draw scroll indicator if needed (moved up due to taller box)
    ...
    def handle_journal_cursor_click(self, pos):
        ...
        if not display_lines:
            self.set_journal_cursor_position(len(current_text))
            return

        # Calculate which display line was clicked
        relative_y = max(0, pos[1] - self.journal_content_rect.top - 5)
        line_height = 22
        ...
        line_text = line_meta.get("text", "")
        line_start = line_meta.get("start", 0)

        # Determine character offset within the line
        ...
        new_position = line_start + char_offset
        new_position = max(0, min(len(current_text), new_position))
        self.set_journal_cursor_position(new_position)
    ...
    def draw_actions_menu(self, screen):
        ...
        # Draw bladesong tracker on the right
        self.draw_bladesong_tracker(screen)
    ...
    def draw_action_info_box(self, screen):
        ...
        if self.selected_action == "bladesong":
            current_y = card_y + 10
            status_text = "Active" if self.bladesong_active else "Inactive"
            status_color = (90, 130, 160) if self.bladesong_active else TEXT_SECONDARY
            status_surface = self.text_font.render(f"Status: {status_text}", True, status_color)
            screen.blit(status_surface, (x + 20, current_y))
            current_y += 30

            uses_text = f"Uses Remaining: {self.bladesong_uses_remaining}/{self.bladesong_max_uses}"
            uses_surface = self.text_font.render(uses_text, True, TEXT_PRIMARY)
            screen.blit(uses_surface, (x + 20, current_y))
            current_y += 28

            int_modifier = self.stats.get("INT", {}).get("modifier", 0)
            bladesong_bonus = max(1, int_modifier)
            bonuses = [
                f"+{bladesong_bonus} AC (INT modifier)",
                "+10 ft speed",
                "ADV on Acrobatics checks",
                "Use INT for attack & damage rolls",
                f"+{bladesong_bonus} CON saves (concentration)"
            ]
        else:
            current_y = card_y + 10
            bonuses = []
        
        bullet_prefix = self.small_font.render("• ", True, TEXT_SECONDARY)
        bullet_width = bullet_prefix.get_width()
        max_bonus_width = width - 40 - bullet_width
        for bonus in bonuses:
            wrapped_bonus = self.wrap_text(bonus, self.small_font, max_bonus_width)
            for idx, line in enumerate(wrapped_bonus):
                if idx == 0:
                    screen.blit(bullet_prefix, (x + 20, current_y))
                    bonus_surface = self.small_font.render(line, True, TEXT_SECONDARY)
                    screen.blit(bonus_surface, (x + 20 + bullet_width, current_y))
                else:
                    continuation_surface = self.small_font.render(line, True, TEXT_SECONDARY)
                    screen.blit(continuation_surface, (x + 20 + bullet_width, current_y))
                current_y += 20

            reminder_text = "No armor or shield, one-handed attacks only."
            reminder_surface = self.small_font.render(reminder_text, True, TEXT_SECONDARY)
            screen.blit(reminder_surface, (x + 20, card_y + height - 35))
            return
    ...
    def draw_bladesong_tracker(self, screen):
        """Show Bladesong uses and status on the right side of the Actions menu."""
        if "bladesong" not in self.actions:
            return
        ...
        x = self.current_width - 220
        y = 150
        max_uses = max(0, self.bladesong_max_uses)
        
        card_y = self.draw_card(screen, x, y, width, height, "Bladesong", "center", 200)
        current_y = card_y + 15
        
        status_text = "ACTIVE" if self.bladesong_active else "Ready" if self.bladesong_uses_remaining > 0 else "Exhausted"
        status_color = (90, 130, 160) if self.bladesong_active else (80, 140, 80) if self.bladesong_uses_remaining > 0 else (160, 80, 80)
        status_surface = self.text_font.render(status_text, True, status_color)
        screen.blit(status_surface, (x + 20, current_y))
        current_y += 35
        
        uses_text = f"{self.bladesong_uses_remaining}/{max_uses} Uses" if max_uses > 0 else "No usable charges"
        uses_surface = self.text_font.render(uses_text, True, TEXT_PRIMARY)
        screen.blit(uses_surface, (x + 20, current_y))
        current_y += 35
        
        # Draw use indicators similar to spell slots
        slot_size = 20
        spacing = 10
        if max_uses > 0:
            total_width = max_uses * slot_size + (max_uses - 1) * spacing
            start_x = x + (width - total_width) // 2
            
            for i in range(max_uses):
                slot_x = start_x + i * (slot_size + spacing)
                slot_y = current_y
                color = (255, 165, 80) if i < self.bladesong_uses_remaining else (60, 60, 60)
                pygame.draw.circle(screen, color, (slot_x + slot_size // 2, slot_y + slot_size // 2), slot_size // 2)
                pygame.draw.circle(screen, TEXT_SECONDARY, (slot_x + slot_size // 2, slot_y + slot_size // 2), slot_size // 2, 2)
            
            current_y += slot_size + 20
        else:
            current_y += 30
        
        int_modifier = self.stats.get("INT", {}).get("modifier", 0)
        bladesong_bonus = max(1, int_modifier)
        bonus_text = f"AC +{bladesong_bonus} | ATK: INT"
        bonus_surface = self.text_font.render(bonus_text, True, TEXT_PRIMARY)
        screen.blit(bonus_surface, (x + 20, current_y))
        current_y += 28
        
        reminder_surface = self.small_font.render("Recharge: Long Rest / AR", True, TEXT_SECONDARY)
        screen.blit(reminder_surface, (x + 20, current_y))
    ...
    def perform_action(self):
        ...
        if not getattr(self, "current_action_enabled", True) and self.selected_action != "bladesong":
            logging.warning("This action is not currently available.")
            return
        
        action_data = self.actions[self.selected_action]
        
        if self.selected_action == "bladesong":
            if not self.bladesong_active and self.bladesong_uses_remaining <= 0:
                logging.warning("No Bladesong uses remaining. Take a long rest to recharge.")
                return
            toggled = self.toggle_bladesong()
            if toggled:
                self.last_action_roll = None
                self.create_action_buttons()
            return
